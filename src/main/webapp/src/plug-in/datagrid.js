    ...
    Datagrid.prototype = {
        ...
        showWin:function(e){
            ...
            if ($(e.currentTarget).hasClass('add')) {
                $title.html('添加操作');
                this.$operateForm.attr({action:this.options.url+'/get'});
            } else if($(e.currentTarget).hasClass('delete')){
                if (this.reloadForm()) return false;
                $title.html('删除操作');
                this.$operateForm.attr({action:this.options.url+'/delete'});
            } else if($(e.currentTarget).hasClass('modify')||$(e.currentTarget).is("tr")){
                if (this.reloadForm()) return false;
                $title.html('修改操作');
                this.$operateForm.attr({action:this.options.url+'/post'});
            }
